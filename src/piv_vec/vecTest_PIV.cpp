      ...
      for(int jj= ii+1; jj < Natm; jj++) {
          if(jj == Natm - 1) {
            for(int nn = 0; nn < NL_const_size; nn++) {
              Value* comp;
              comp=getPntrToArgument(total_count)->getName()+"_ELEMENT";
              addComponent(comp); 
  ...
  if(getStep()%updatePIV==0) {
    ...
    for(unsigned j=0; j<Nlist; j++) {
      ...
      for(unsigned i=rank; i<limit; i+=stride) {
        ...
        double tmp=0.;
        if(cart2piv) {
          tmp = 2.*scaling[j]*cPIV[j][i]*Fvol*Fvol*dfunc;
        } else {
      ...
      for (int jj = ii+1; jj < Natm; jj ++) {
        if(jj == Natm - 1) {
              for(int nn = 0; nn < NL_const_size; nn ++) {
                ...
                //value_new -> set(cPIV[count][nn]);
                //value_new -> setDerivative(total_count, m_deriv[total_count]);
                total_count += 1;
              }
        } else {
          Value* comp_pnt
          comp_pnt=getPntrToComponent(getPntrToArgument(total_count)->getName()+"_ELEMENT")->set(cPIV[count][0]);
          //string name_of_this_component = getPntrToValue(total_count)->getName()+"_PIV_EL";
          //Value* value_new=getPntrToComponent(name_of_this_component);
          //value_new -> set(cPIV[count][0]);
          //value_new -> setDerivative(total_count, m_deriv[total_count]);
          total_count += 1;
        }
        count += 1;
